...
\vfill
{\bf Q:} This forces the expectation to be positive, but not an integer. Is this a problem?
\vfill
Formallly, the Poisson regression model can be expressed as:
\begin{eqnarray*}
Y|x &\sim& Poisson(\theta_x) = Poisson(\exp(\beta_0 + \beta_1 x + \beta_2 x^2))\\
\end{eqnarray*}
\vfill
In a generalized linear model there are three defining components:
\begin{enumerate}
	\item A probability distribution from an exponential family, in this case the Poisson distribution,\\
	\item a linear predictor ($\beta_0 + \beta_1 x + \beta_2 x^2$) is linear in the predictors,  and \\
	\item a link function, which is the log link in the Poisson case.
\end{enumerate}
\vfill
...
& \propto& \prod_{i=1}^n \exp(\tilde{\beta}^T \tilde{x_i})^y \exp \left( - \exp(\tilde{\beta}^T \tilde{x}_i ) \right) / y_i!  \times \\
&& (2 \pi) ^{-p/2} |\Sigma_0|^{-1} \exp\left(-\frac{1}{2}(\tilde{\beta} - \tilde{\beta}_0)^T \Sigma_0^{-1} (\tilde{\beta} - \tilde{\beta}_0)\right)
\end{eqnarray*}
...
\begin{equation}
\alpha = \frac{\pi(\theta^*)}{\pi(\theta^{(s)})} \times \frac{J(\theta|\theta^*)}{J(\theta^*|\theta)},
\end{equation}
...
\vfill
__A:__ We are less likely to propose a step from 1 to 0 than vice versa, so the the resulting acceptance probability from 1 to 0 includes an inflating factor for the acceptance probability.
\vfill
...
\vfill
1. __irreducible__. A reducible chain will have non-overlapping sets between which the algorithm is unable to move. The textbook cites a proposal mechanism that proposes values $\pm 2$. Hence, the chain would not be able to move from even to odd numbers and vice versa.
\vfill
2. __aperiodic__. An aperiodic chain has values that can only be visited every $k$ observations, which implies that $Pr(x^{(s)} =x) = 0$ for some values.
\vfill
3. __recurrent__. A recurrent chain states that if $x^{(s)}= x$ then $\pi(x^{(s)} >0$ so the chain must be guaranteed to return to $x$, eventually.
\vfill
\newpage
...
\vfill
1. $Pr(x^{(s)} \in A) \rightarrow \pi(A)$ for any set A;
\vfill
2. $\frac{1}{S} \sum g(x^{(s)}) \rightarrow \int g(x) \pi(x) dx$.
\vfill
